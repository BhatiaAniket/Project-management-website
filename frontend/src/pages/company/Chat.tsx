...
import { Send, Search, Plus, Users, MessageCircle, Loader2 } from 'lucide-react';
        ...
        <div className="p-4 border-b border-border">
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="flex bg-muted/50 p-1 rounded-lg">
              <button onClick={() => setActiveTab('chats')} className={`px-3 py-1 text-xs font-medium rounded-md transition-shadow ${activeTab === 'chats' ? 'bg-background shadow-sm' : 'text-muted-foreground hover:text-foreground'}`}>Chats</button>
              <button onClick={() => setActiveTab('contacts')} className={`px-3 py-1 text-xs font-medium rounded-md transition-shadow ${activeTab === 'contacts' ? 'bg-background shadow-sm' : 'text-muted-foreground hover:text-foreground'}`}>Contacts</button>
            </div>
          </div>
          <div className="relative">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
            <input type="text" placeholder={`Search ${activeTab}...`} className="w-full pl-9 pr-3 py-2 rounded-xl border border-border bg-background text-sm focus:outline-none focus:border-foreground" />
          </div>
        </div>
        <div className="flex-1 overflow-y-auto">
          {activeTab === 'contacts' ? (
            contacts.length === 0 ? (
              <div className="text-center py-8"><Users className="w-8 h-8 mx-auto text-muted-foreground mb-2" /><p className="text-sm text-muted-foreground">No contacts found</p></div>
            ) : (
              // Filter out the logged-in user from contacts
              contacts.filter(c => c._id !== user?.id).map((contact) => (
                <button key={contact._id} onClick={() => startChat(contact)} className="w-full text-left px-4 py-3 border-b border-border hover:bg-muted/30 transition-colors flex items-center gap-3">
                  <div className="relative">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground capitalize mt-0.5">{contact.role}</p>
                  </div>
                </button>
              ))
            )
            ...
            loading ? (
              ...
              <div className="text-center py-8"><MessageCircle className="w-8 h-8 mx-auto text-muted-foreground mb-2" /><p className="text-sm text-muted-foreground">No conversations yet</p></div>
            ) : (
              conversations.map((conv) => {
                const isOnline = conv.participants?.some((p: any) => p._id !== user?.id && onlineUsers.includes(p._id));
                return (
                  <button key={conv._id} onClick={() => setActiveConv(conv)} className={`w-full text-left px-4 py-3 border-b border-border hover:bg-muted/30 transition-colors flex items-center gap-3 ${activeConv?._id === conv._id ? 'bg-muted/40' : ''}`}>
                    <div className="relative">
                      <div className="w-10 h-10 rounded-full bg-foreground/10 flex items-center justify-center text-sm font-bold shrink-0">
                        {conv.type === 'group' ? <Users className="w-5 h-5" /> : getConversationName(conv).charAt(0)}
                      </div>
      ...
      <div className="flex-1 flex flex-col">
        {!activeConv ? (
          <div className="flex-1 flex items-center justify-center">
            <div className="text-center"><MessageCircle className="w-12 h-12 mx-auto text-muted-foreground mb-3" /><p className="text-lg font-semibold">Select a conversation</p><p className="text-sm text-muted-foreground">Choose from your contacts to start chatting</p></div>
          </div>
        ) : (
          <>
                ...
                messages.map((msg, i) => {
                  ...
                  return (
                    <div key={msg._id || i} className={`flex ${isMine ? 'justify-end' : 'justify-start'}`}>
                      <div className={`max-w-[70%] px-4 py-2.5 rounded-2xl text-sm ${isMine ? 'bg-foreground text-background rounded-br-md' : 'bg-muted rounded-bl-md'}`}>
            ...
            <div className="p-4 border-t border-border">
              <div className="flex gap-2">
                <input type="text" value={newMessage} onChange={(e) => { setNewMessage(e.target.value); handleTyping(); }} onKeyDown={(e) => e.key === 'Enter' && sendMessage()} placeholder="Type a message..." className="flex-1 px-4 py-2.5 rounded-xl border border-border bg-background text-sm focus:outline-none focus:border-foreground" />
                <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={sendMessage} disabled={!newMessage.trim()} className="p-2.5 rounded-xl bg-foreground text-background disabled:opacity-50">
