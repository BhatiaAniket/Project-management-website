...
// We use the generic chat endpoints mounted on /api/chat which the companyAPI client exposes
...
export default function EmployeeChat() {
  ...
  useEffect(() => {
    const fetchConvs = async () => {
      ...
      } catch { showToast('Error loading chats', 'error'); }
  ...
  const getConversationName = (conv: any) => {
    if (conv.type === 'company' || conv.name) return conv.name || 'Company Chat 🏢';
        ...
        <div className="p-4 border-b border-border">
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="flex bg-muted/50 p-1 rounded-lg">
              ...
              <button onClick={() => setActiveTab('contacts')} className={`px-3 py-1 text-xs font-medium rounded-md ${activeTab === 'contacts' ? 'bg-background shadow-sm text-foreground' : 'text-muted-foreground hover:text-foreground'}`}>Colleagues</button>
            </div>
          </div>
          ...
          {activeTab === 'contacts' ? (
            contacts.length <= 1 ? (
              ...
              contacts.filter(c => c._id !== user?.id).map((contact) => (
                <button key={contact._id} onClick={() => startChat(contact)} className="w-full text-left px-4 py-3 border-b border-border hover:bg-muted/30 flex items-center gap-3 transition-colors">
                  <div className="relative">
            ...
            loading ? (
               ...
               conversations.map((conv) => {
                 ...
                 return (
                   <button key={conv._id} onClick={() => setActiveConv(conv)} className={`w-full text-left px-4 py-3 border-b border-border hover:bg-muted/30 flex items-center gap-3 transition-colors ${activeConv?._id === conv._id ? 'bg-muted/40' : ''}`}>
                     <div className="relative">
                       <div className="w-10 h-10 rounded-full bg-blue-500/10 text-blue-600 flex items-center justify-center text-sm font-bold shrink-0">{getConversationName(conv).charAt(0)}</div>
      ...
      <div className="flex-1 flex flex-col bg-muted/10 relative">
        {!activeConv ? (
          <div className="flex-1 flex items-center justify-center text-center">
            <div><MessageCircle className="w-12 h-12 mx-auto text-muted-foreground mb-3" /><p className="text-lg font-semibold">Select a conversation</p><p className="text-sm text-muted-foreground">Communicate with your team</p></div>
          </div>
        ) : (
          <>
            <div className="p-4 border-b border-border flex items-center gap-3 bg-card shadow-sm z-10">
              <div className="w-8 h-8 rounded-full bg-blue-500/10 text-blue-600 flex items-center justify-center text-sm font-bold shadow-sm">{getConversationName(activeConv).charAt(0)}</div>
              <div><p className="text-sm font-semibold">{getConversationName(activeConv)}</p>{typingUser && <p className="text-[10px] text-muted-foreground animate-pulse">{typingUser} is typing...</p>}</div>
            </div>
            ...
            <div className="p-4 border-t border-border bg-card">
              <div className="flex gap-2 items-center">
                ...
                <input type="text" value={newMessage} onChange={(e) => { setNewMessage(e.target.value); handleTyping(); }} onKeyDown={(e) => e.key === 'Enter' && sendMessage()} placeholder="Send a message..." className="flex-1 px-4 py-2.5 rounded-full border border-border bg-background text-sm focus:outline-none focus:border-foreground" />
                <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={sendMessage} disabled={!newMessage.trim()} className="p-2.5 rounded-full bg-foreground text-background disabled:opacity-50 shadow-sm">
