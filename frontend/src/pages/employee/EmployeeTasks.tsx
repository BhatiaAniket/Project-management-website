...
import { AlertCircle, Calendar, CheckSquare, Clock, AlignLeft, GripVertical, Loader2 } from 'lucide-react';
...
const EmployeeTasks = () => {
  ...
  const onDragEnd = async (result: any) => {
    ...
    try {
      await employeeAPI.updateTaskStatus(draggableId, newStatus);
      if (newStatus === 'completed') {
        const t = previousTasks.find(x => x._id === draggableId);
        showToast('Task marked as done. Please submit a daily report.', 'success');
        navigate('/employee/reports', { state: { prefilledTask: draggableId } });
      }
    } catch (e) {
  ...
  const getUrgencyColor = (dateString: string) => {
    ...
    if (d.getTime() < now.getTime()) return 'text-red-500'; // overdue
    if (d.getTime() === now.getTime()) return 'text-yellow-500'; // today
                        ...
                        <Draggable key={task._id} draggableId={task._id} index={index}>
                          {(provided, snapshot) => (
                            ...
                            >
                              ...
                              <p className="text-xs text-muted-foreground mb-3 line-clamp-2">{task.project?.name}</p>

                              <div className="flex items-center justify-between text-xs mt-auto">
                                <div className={`flex items-center gap-1 font-medium ${getUrgencyColor(task.dueDate)}`}>
